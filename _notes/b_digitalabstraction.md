...
>   <span style="background-color:yellow; color: black"> VTC **does not** tell us how fast the device is. It just captures the static behavior of the device and tells us what *kind* of device it is.
> </span>
...
1.  There exist *some region in the VTC* whereby   <span style="background-color:yellow; color: black">its **absolute** `Gain` is $$>1$$ </span>. `Gain` is actually a function of $$V_{in}$$ and is **formally** defined as: 
